...
# -----------------------------------------------------------------------------
    ...
    st.info("💡 Este módulo toma los documentos base (ej. Inventarios o Matrices de 2025), actualiza sus fechas internamente a 2026 en memoria y te permite descargar la copia lista para colocarla en la carpeta 'Auditoría actual'.")
    
    if st.button("🔄 Generar actualización de Matriz de Riesgos y Cronogramas a 2026"):
        with st.spinner("Procesando datos desde la nube... identificando fechas 2025... actualizando a 2026..."):
            # Aquí va la lógica de Pandas para leer el Excel base, sumarle 1 año a las fechas y exportarlo
            time.sleep(2.5) # Simulamos el proceso de pandas
            
            # Al terminar, habilitamos el botón de descarga real de Streamlit
            st.success("¡Documentos analizados y actualizados exitosamente en memoria!")
            
            # Simulamos el archivo procesado (En un caso real exportamos el DataFrame a Excel)
            st.download_button(
                label="📥 Descargar Matriz_Riesgos_Vigencia_2026.xlsx",
                data=b"Simulacion de archivo xlsx binario",
